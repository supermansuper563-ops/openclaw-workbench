...
## Workbench fork reporting

This repository adds downstream Workbench surfaces to OpenClaw. Use this repository's
[private vulnerability report](../../security/advisories/new) for a sensitive finding in:

- `OpenClaw-Workbench-Setup.cmd` or `workbench/` source setup;
- the Control UI under `ui/src/pages/workbench/`;
- `extensions/workbench-guardrails/`; or
- a security claim made only by Workbench documentation.

If the private form is unavailable, do not publish the exploit or proof in an issue. Open a
non-sensitive issue that names only the affected Workbench component and asks for a confidential
coordination route. Do not attach credentials, personal data, unredacted journals, or reproduction
details that reveal the vulnerability.

Use the upstream route below when the same problem exists in an unmodified OpenClaw revision.
Workbench navigation, setup, or policy behavior is downstream-owned even though this fork retains
upstream history. The [Workbench support scope](docs/workbench/index.md#support-and-security-scope)
has the full routing table.
