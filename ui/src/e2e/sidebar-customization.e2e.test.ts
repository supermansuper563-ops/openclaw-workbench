...
describeControlUiE2e("Control UI sidebar customization mocked Gateway E2E", () => {
  ...
  it("pins routes, restores defaults, and persists navigation state across reloads", async () => {
    ...
    try {
      ...
      await expect.poll(() => trimmedTextContents(pinnedItems)).toEqual(["Workbench", "Tasks"]);
      ...
      await expect
        ...
        .not.toContain("Workboard");
      const automationsItem = menu.getByRole("menuitemcheckbox", { name: "Automations" });
      await expect.poll(() => automationsItem.getAttribute("aria-checked")).toBe("false");
      ...
      await expect
        ...
        .toEqual(["Workbench", "Tasks", "Automations"]);
      await page.reload();
      await expect
        .poll(() => trimmedTextContents(pinnedItems))
        .toEqual(["Workbench", "Tasks", "Automations"]);
