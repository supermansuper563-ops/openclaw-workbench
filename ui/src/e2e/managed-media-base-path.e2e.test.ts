...
describe("Control UI managed media under a UI base path", () => {
  ...
  it("keeps origin-root managed-media APIs outside the UI base path", async () => {
    ...
    try {
      // The Vite harness serves its SPA fallback at every path. Navigate through
      // the configured mount prefix so routing and media URLs exercise the same
      // reverse-proxy shape used by Gateway.
