...
import { page as workbenchPage } from "./pages/workbench/route.ts";
...
export async function startApplicationRouter(
  ...
  // Workbench is this distribution's front door. Unknown paths, including the
  // dashboard root and retired routes, land on its guided setup surface.
  if (routeIdFromPath(location.pathname, basePath) === null) {
    history.replace({
      ...location,
      pathname: router.pathForRoute("workbench", basePath),
    });
    location = history.location();
  }
