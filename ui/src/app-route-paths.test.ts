...
describe("Workbench startup route", () => {
  it.each(["/", "/overview"])("redirects %s to the Workbench front door", async (pathname) => {
    let location: RouteLocation = { pathname, search: "?probe=1", hash: "#start" };
    ...
    const router = createApplicationRouter();
    const route = router.getRoute("workbench");
    if (!route) {
      throw new Error("Workbench route missing");
    }
    const originalLoader = route.loader;
    const originalComponent = route.component;
    try {
      route.loader = async () => undefined;
      route.component = async () => ({ render: () => null });
